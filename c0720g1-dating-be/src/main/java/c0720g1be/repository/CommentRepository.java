...
public interface CommentRepository extends JpaRepository<Comment, Integer> {
    ...
    @Query(value = "SELECT * FROM comment where comment.comment_id = ?1 and delete_flag = false order by date_comment DESC", nativeQuery = true)
    Page<Comment> findAllByCommentReply(Integer idReply, Pageable pageable);

    @Modifying
    @Query(value = "INSERT INTO `comment`(`content`,`date_comment` ,`account_id`, `delete_flag` ,`comment_id`) " +
            "value (?1, ?2, ?3, ?4, ?5) ", nativeQuery = true)
    void createReplyComment(String content, String dateComment, Integer accountId, Boolean deleteFlag, Integer commentId);
